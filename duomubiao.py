...
def start_osai_service(exe_path):
    ...
    try:
        ...
        main_window = app.window(title="OSAI_HTL_Plugin")
        return main_window
...
def click_function_button(window_obj, times):
    ...
    try:
        # 1. 预先定位按钮 (只找一次，确保速度)
        target_button = window_obj.child_window(auto_id="MultipleWidget.widget_background.widget_center.widget_center_right.widget_center_right_tool.pushButton_reidentify",control_type="Button")
...
if __name__ == "__main__":
    # 定义你的程序路径
    MY_EXE = r"D:\OSAI_HTL_Plugin\OSAI_HTL_Plugin.exe"
    # 第一步：连接程序
